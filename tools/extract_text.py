...
# 這裡把已考訂出對應字的還原回去；未列入的仍維持原樣。
# 前面幾組取的是原刻字形，最後一組原刻字形 Unicode 查無，改取通行字。
MISSING_GLYPHS = {
    # 罵詈、動作
    "［扌日］": "抇",  # 穩抇抇。通行作「拍」
    "［扌造］": "𢳥",  # 堅硬𢳥掘。通行作「崛」
    ...
    "［歹帶］": "殢",     # 殢雨尤雲
    "［髟參］": "鬖",     # 黑鬖鬖
    "［悤頁］": "𩕄",     # 𩕄門（顖門）。𩕄＝⿰悤頁，與原註相符；通行作「顖」
    "［疒羅］": "㿚",     # 㿚＝疒+羅（癳是疒+纍，構字不合）
    "［毛俞］": "毺",     # 氈毺。毺＝⿺毛俞，與原註相符；通行作「毹」（⿰俞毛）
    "［毛戊］": "𣬽",     # 𣬽＝⿺毛戊，與原註相符；通行作「毧」（⿺毛戎）
    "［兀王］": "尫",     # 尫羸。尫＝兀+王，與原註相符（尪從尢）
    # 言語、聲響
    "［門坐］": "䦟",  # 與「䦛」連用
    "［氵芻］": "𣺥",  # 韻文擬聲「滑滑𣺥𣺥」
    "［口樂］": "嚛",     # 歎詞
    ...
    "［釒劉］": "𨮸",  # 𨮸下一兩五錢。通行作「鎦」
    "［糹至］": "絰",     # 孝絰
    "［囗扁］": "𱖄",     # 𱖄金補子。𱖄＝⿴囗扁，與原註相符；通行作「匾」（⿷匚扁）
    "［秋瓦］": "甃",     # 石甃
    "［石乞］": "矻",     # 矻磴
    "［竹貢］": "篢",     # 虛篢
    "［“蝶”“蟲”改“火”］": "煠",  # 油炸（原註為「蝶」字蟲旁改火）
    # 擴充 H（Unicode 15.0，2022）才收入的字，同樣取原刻字形
    "［糹堂］": "𱺃",     # 人名，文嫂之子「文𱺃」
    "［石否］": "𱴁",     # 內有一根𱴁折了
    "［禾囗也］": "𱶄",   # 一泡𱶄谷都的熱屎
    "［飠皆］": "𲋜",     # 與下字連用，𲋜𲋕樣子肉
    ...
    "［扌欒］": "𱡖",     # 𱡖箏歌板。通行作「擽」
    "［石店］": "𱴗",     # 𱴗言試語。通行作「玷」
    "［疒各］": "𱱓",     # 使的那臉𱱓。字書未載，第一奇書云「有書以闊代之」
    "［扌闔］": "𱡐",     # 牙花已𱡐了。通行作「磕」
    "［革反］": "𲊛",     # 減𲊛仙人。通行作「鈑」
    # 以下六種原刻字形整個 IDS 資料庫都查不到，只能取通行字
    "［火誇］": "烤",     # 頓爛烤蹄兒。原刻作火+誇
    "［足褻］": "躞",     # 與𨈈連用作「蹀躞」。原刻作足+褻
    "［足佔］": "踮",     # 頻將腳踮。原刻作足+佔
    "［口薄］": "礴",     # 譁哩礴喇。原刻作口+薄
    "［彳胤］": "胤",     # 童天胤。清刻避雍正帝諱的改形字
    "［“胤”換“丿”為“彳”］": "胤",  # 續箕裘之胤嗣。同上
    # 名物
    "［涑鳥］": "𪄠",  # 鸂𪄠。通行作「鶒」
    "［蟲麻］": "䗫",  # 蝍䗫。䗫即「蟆」之異體
    "［角京］": "𧤀",  # 𧤀角。通行作「犄」
