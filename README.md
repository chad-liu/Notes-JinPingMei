...
處理成三類（對照表在 `tools/extract_text.py` 的 `MISSING_GLYPHS`，補一行再重跑
`extract_text.py` 與 `build_data.py` 即可生效）：

- **依原刻字形還原** 55 種、155 處
- **原刻字形 Unicode 查無，改用通行字** 11 種、14 處（擽、玷、闊、烤、躞、磕、踮、礴、鈑、胤）
- **原刻與通行字皆未定，維持標記原樣** 5 種、8 處
...
由上下文成詞判定者標「辭例」。查無字形的那 16 種，另以 ctext 的《金瓶梅第一奇書》
（同屬崇禎本系統，缺字逐一標成「●缺字：左「糸」右「堂」」）交叉印證過，
標缺字之處與本站完全一致。

清單有「對照」一欄：第一類記通行本的另一種寫法（如 𫄉綢／潞綢、𧤀角／犄角），
第二、三類記原註的構字，以便回溯原刻字形。
